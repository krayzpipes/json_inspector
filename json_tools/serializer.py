...
def serialize_json(obj, format=None):
    """
    ...
    # Recursive cases
        ...
        try:
            ...
            return serialize_json(json.loads(obj))
        except Exception:
        ...
        if obj:
            return [serialize_json(each) for each in obj]
        else:
            return []
        ...
        for key in new_obj.keys():
            new_obj[key] = serialize_json(new_obj[key])
        return new_obj
...
def serialize_json_from_file(file_loc):
    with open(file_loc, 'r') as f:
        lines = f.readlines()
    return [serialize_json(line) for line in lines]
